    ...
    <div>
        ...
        <admin-aside>
            ...
            <div slot="asideContent">
                <div class=" aside_bar">
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <router-link class="btn btn-primary mr-1"
                                     :to="{name:'adminTitleCreate'}" v-if=" $root.$data.createPermission">
                            +Add
                        </router-link>
                ...
                <div class="listContent" ref="listHeight" :style="" v-if="titles.length">
                    <ul class="list-group">

                        <router-link v-for="title in titles"
                                     class="list-group-item" :key="title.id"
                                     :to="{name:'adminTitleShow', params:{id:title.id}}"
                                     @click.native="show(title.id)"
                                     tag="li"
                                     style="cursor:pointer"
                        >
                            {{title.name|asideTitlify}}
                        </router-link>
            ...
            <div slot="asideMainContent">
                <div v-if="title.id">
                    <div class="card-header d-flex justify-content-between flex-wrap">
                        ...
                        <div v-if="title.id">
                            <router-link :to="{name:'adminTitleEdit', params:{id:title.id}}">
                                <i class="mdi mdi-edit"></i>
                            </router-link>
                            <a href="" class="pl-2 default-color" @click.prevent="deleteMe(title.id)">
                    ...
                    <div class="card-body  card-header-contrast">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-body">
                                        <p class="cat_desc" v-if="title.name">
                                            <span>Name </span> <br>
                                            <strong>{{title.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="title.created_at">
                                            <span>Date Created </span> <br>
                                            <strong>{{$root.parseDate(title.created_at)}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="title.updated_at">
                                            <span>Date Modified </span> <br>
                                            <strong>{{$root.parseDate(title.updated_at)}}</strong>
                                        </p>
                                        <div class="cat_desc">
                                            <span>Status </span> <br>
                                            <div class="switch-button switch-button-success switch-button-xs activeSwitch" >
                                                <input
                                                        ...
                                                        name="swt1"
                                                        :id="title.id"
                                                        @click.prevent="$root.changeIsBooleanStatus(title, 'is_active', 'App-Title')"
                                                >
    ...
    export default {
      ...
      updated(){
        ...
        methods:{
            ...
            show(id){
                axios.get('/admin/company/title/'+id,{
            ...
            deleteMe(id){
                this.$root.confirmationDelete().then(val => {
                    if (val) {
                        axios.delete('/admin/company/title/' + id).then(response => {
