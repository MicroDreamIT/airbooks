    ...
    <div>
        ...
        <admin-aside>
            ...
            <div slot="asideContent">
                <div class="aside_bar" >
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <router-link class="btn btn-primary mr-1"
                                     :to="{name:'adminReleaseCreate'}" v-if=" $root.$data.createPermission">
                            +Add
                        </router-link>
                ...
                <div class="listContent" ref="listHeight" :style="" v-if="releases.length">
                    <ul class="list-group">
                        <router-link v-for="release in releases"
                                     class="list-group-item" :key="release.id"
                                     :to="{name:'adminReleaseShow', params:{id:release.id}}"
                                     @click.native="show(release.id)"
                                     tag="li"
                                     style="cursor:pointer"
                        >
                            {{release.name|asideTitlify}}
                        </router-link>
            ...
            <div slot="asideMainContent">
                <div v-if="release.id">
                    <div class="card-header d-flex justify-content-between flex-wrap">
                        ...
                        <div v-if="release.id">
                            <router-link :to="{name:'adminReleaseEdit', params:{id:release.id}}">
                                <i class="mdi mdi-edit"></i>
                            </router-link>
                            <a href="" class="pl-2 default-color" @click.prevent="deleteMe(release.id)">
                    ...
                    <div class="card-body  card-header-contrast">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-body">
                                        <p class="cat_desc" v-if="release.name">
                                            <span>Name  </span> <br>
                                            <strong>{{release.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="release.created_at">
                                            <span>Date Created </span> <br>
                                            <strong>{{$root.parseDate(release.created_at)}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="release.updated_at">
                                            <span>Date Modified </span> <br>
                                            <strong>{{$root.parseDate(release.updated_at)}}</strong>
                                        </p>
                                        ...
                                        <div class="cat_desc">
                                            ...
                                            <div class="switch-button switch-button-success switch-button-xs activeSwitch" >
                                                <input
                                                        ...
                                                        name="swt1"
                                                        :id="release.id"
                                                        @click.prevent="$root.changeIsBooleanStatus(release, 'is_active', 'App-Release')"
                                                >
                                                <span><label :for="release.id"></label></span>
                                            </div>
                                        </div>
...
<script>
    export default {
        ...
        methods:{
            ...
            show(id){
                axios.get('/admin/parts/release/'+id,{
            ...
            deleteMe(id){
                this.$root.confirmationDelete().then(val => {
                    if (val) {
                        axios.delete('/admin/parts/release/' + id).then(response => {
