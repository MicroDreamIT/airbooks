    ...
    <div>
        ...
        <admin-aside>
            ...
            <div slot="asideContent">
                ...
                <div class="listContent"  ref="listHeight" :style=""v-if="cmses.length">
                    <ul class="list-group">

                        <router-link v-for="cms in cmses"
                                     class="list-group-item" :key="cms.id"
                                     :to="{name:'adminCmsShow', params:{id:cms.id}}"
                                     @click.native="show(cms.id)"
                                     tag="li"
                                     style="cursor:pointer"
                        >
                            {{cms.title|asideTitlify}}
                        </router-link>
            ...
            <div slot="asideMainContent">
                <div v-if="cms.id">
                    <div class="card-header d-flex justify-content-between flex-wrap">
                        <span>{{cms.name}}</span>
                        <div v-if="cms.id">
                            <router-link :to="{name:'adminCmsEdit', params:{id:cms.id}}">
                                <i class="mdi mdi-edit"></i>
                            </router-link>
                            <a class="pl-2 pr-2" @click.prevent="deleteMe(cms.id)" href="">
                    ...
                    <div class="card-body  card-header-contrast">
                        <div class="row">
                            <div class="col-md-9">
                                <div class="card">
                                    <div class="card-body">
                                        <p class="cat_desc" v-if="cms.url">
                                            <span>Page Url </span> <br>
                                            <strong>{{cms.url}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="cms.title">
                                            <span>Title </span> <br>
                                            <strong>{{cms.title}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="cms.sub_title">
                                            <span>Sub Title </span> <br>
                                            <strong>{{cms.sub_title}}</strong>
                                        </p>

                                        <p class="cat_desc" v-if="cms.section">
                                            <span>Section </span> <br>
                                            <strong>{{cms.section}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="cms.custom_url">
                                            <span>Page Url </span> <br>
                                            <strong>{{cms.custom_url}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="cms.body">
                                            <span>Content Body </span> <br>
                                            <strong v-html="cms.body"></strong>
                                        </p>
                                        <div class="cat_desc">
                                            <span>Status </span> <br>
                                            <div class="switch-button switch-button-success switch-button-xs activeSwitch" >
                                                <input
                                                        type="checkbox"
                                                        :checked="cms.is_active"
                                                        name="swt1"
                                                        :id="cms.id"
                                                        @click.prevent="$root.changeIsBooleanStatus(cms, 'is_active', 'App-Cms')"
                                                >
                                                <span><label :for="cms.id"></label></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="darea" role="alert">
                                    <div class="message" v-if="cms.medias">
                                        <img :src="'/storage/'+cms.medias.path+'/'+cms.medias.original_file_name" class="img-fluid d-block mx-auto" >
                                    </div>
        ...
        methods:{
            ...
            deleteMe(id){
                this.$root.confirmationDelete().then(val => {
                    if (val) {
                        axios.delete('/admin/cms/' + id).then(response => {
