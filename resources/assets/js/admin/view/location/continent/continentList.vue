<template>
    <div class="card card-table">
        <data-table
                ...
                :search="true"
                :columns="columns"
                :addLink="'adminContinentCreate'"
                :tableHeadline="'Continents'"
                :searchField="'name'"
                :modelName="'continent'"
                :ajax="{
                            url: url
        ...
        >
            <template slot="items" slot-scope="props">
                ...
                <td>
                    <router-link :to="{name:'adminContinentShow', params:{id:props.data.id}}">
                        {{props.data.name}}
                    </router-link>
                </td>
                <td>
                    <div class="switch-button switch-button-success switch-button-xs activeSwitch" v-if="$root.$data.editPermission" >
                        <input
                                ...
                                name="swt1"
                                :id="props.data.id"
                                @click.prevent="$root.changeIsBooleanStatus(props.data, 'is_active', 'App-Continent')"
                        >
                ...
                <td>
                   <div class="adjustAction">
                       <router-link :to="{name:'adminContinentEdit', params:{id:props.data.id}}" v-if="$root.$data.editPermission">
