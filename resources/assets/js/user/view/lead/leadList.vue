                ...
                <td class="leads-action">
                    <div>
                        <div class="adjustAction">
                            <a @click.prevent="callLeadLIst(props.data)"  href="">
                                <i class="mdi mdi-eye"></i>
                            </a>
                        </div>
                        <div class="adjustAction">
                            <a @click.prevent="deleteMe(props.data)" href="">
        ...
        <div v-if="leadList" class="lead-List-Modal">
            ...
            <div class="card w-50 lead-list-area">
                ...
                <div class="card-footer text-muted">
                       <button class="btn btn-primary float-right" @click.prevent="closeLeads">Close</button>
                </div>
        ...
        methods: {
              ...
              this.leadList = true
                ...
                this.modelInfo=item;
                  if(item.lead_status=='Unread'){
                      axios.patch('/user/ajax/lead/'+ item.id, item).then(res=>{
            ...
            deleteMe(data) {
                this.$root.confirmationDelete().then(val => {
                    if (val) {
                        axios.delete('/user/ajax/lead/' + data.id,{
                            data:data,
                        }).then(response => {
                            var dataTable = this.$refs.dataTable
                            dataTable.getResult();
