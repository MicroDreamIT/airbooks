    ...
    <div>
        <admin-aside>
            <div slot="asideContent">
                <div class="aside_bar">
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <router-link class="btn btn-primary mr-1"
                                     :to="{name:'adminNewsCreate'}" v-if=" $root.$data.createPermission">
                            +Add
                        </router-link>
                ...
                <div class="listContent" ref="listHeight" :style="" v-if="newsList.length">
                    <ul class="list-group">

                        <router-link v-for="news in newsList"
                                     class="list-group-item" :key="news.id"
                                     :to="{name:'adminNewsShow', params:{id:news.id}}"
                                     @click.native="show(news.id)"
                                     tag="li"
                                     style="cursor:pointer"
                        >
                            {{news.title|asideTitlify}}
                        </router-link>
                ...
                <div v-if="news.id">
                    <div class="card-header d-flex justify-content-between flex-wrap">
                        ...
                        <div v-if="news.id">
                            ...
                            <a href="" class="pl-2 default-color" @click.prevent="deleteMe(news.id)">
                    ...
                    <div class="card-body  card-header-contrast">
                        <div class="row">
                            <div class="col-md-9">
                                <div class="card">
                                    <div class="card-body">
                                        <p class="cat_desc" v-if="news.title">
                                            <span>Title  </span> <br>
                                            <strong>{{news.title}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="news.date">
                                            <span>Date  </span> <br>
                                            <strong>{{$root.parseDate(news.date)}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="news.categories" >
                                            <span>Category  </span> <br>
                                            <template v-for="(category, index) in news.categories">
                                            <strong>{{category.name}}</strong>
                                            <strong v-if="index < news.categories.length-1 "> | </strong>
                                            </template>
                                        </p>
                                        <p class="cat_desc" v-if="news.company">
                                            <span>Company  </span> <br>
                                            <strong> {{news.company.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="news.continent">
                                            <span>Continent  </span> <br>
                                            <strong> {{news.continent.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="news.region">
                                            <span>Region  </span> <br>
                                            <strong> {{news.region.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="news.country">
                                            <span>Country  </span> <br>
                                            <strong> {{news.country.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="news.source">
                                            <span>Description  </span> <br>
                                            <strong> {{news.source}}</strong>
                                        </p>

                                        <p class="cat_desc" v-if="news.details">
                                            <span>Details</span> <br>
                                            <strong v-html="news.details"></strong>

                                        </p>
                                        <p class="cat_desc" v-if="news.created_at">
                                            <span>Date Created </span> <br>
                                            <strong>{{$root.parseDate(news.created_at)}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="news.updated_at">
                                            <span>Date Modified </span> <br>
                                            <strong>{{$root.parseDate(news.updated_at)}}</strong>
                                        </p>
                                        <div class="cat_desc">
                                            <span>Status </span> <br>
                                            <div class="switch-button switch-button-success switch-button-xs activeSwitch" >
                                                <input
                                                        ...
                                                        name="swt1"
                                                        :id="news.id"
                                                        @click.prevent="$root.changeIsBooleanStatus(news, 'is_active', 'App-News')"
                                                >
                                                <span><label :for="news.id"></label></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="col-md-3">
                                <div class="darea" role="alert" >
                                    <div class="message" v-if="news.medias">
                                        <img :src="'/storage/'+news.medias.path+'/'+news.medias.original_file_name" class="img-fluid d-block mx-auto" >
                                    </div>
