export default {
    ...
    computed:{
        ...
        metas(){
            this.$root.setupSeo(this.$store.state.seos, 'Apu')
            return !!this.$store.state.seos.length
        },

        chunkFeatured(){

            let apus = this.$root.findObjectByKey_in_array_and_return_them_array(this.$root.$data.featured, 'modelType', 'apu')

            return _.chunk(_.take(apus, 3), 3)
        }
    },
    ...
    methods:{
            ...
            axios.post('/user/ajax/likes',{id:modelId, path:modelPath})
                .then(res=>{
                    if(res.data.unlike){
                        ...
                        let indexNo = items.likes.findIndex(item=> item.likable_id == modelId && item.user_id == this.$root.$data.user.id)
        ...
        getData(current_page){
            ...
            axios.get('/ajax/apu?page='+current_page+'&resultPerPage=20&sortOrder=desc&sortName=id&is_active_by_user=1&isActiveStatus=Approved&sortOrder=desc&paging=true&title='+this.title).then(res=>{
