    ...
    <div>
        ...
        <admin-aside>
            ...
            <div slot="asideContent">
                <div class=" aside_bar" >
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <router-link class="btn btn-primary mr-1"
                                     :to="{name:'adminAirfieldCreate'}" v-if=" $root.$data.createPermission">
                            +Add
                        </router-link>
                ...
                <div class="listContent" v-if="airfields.length">
                    <ul class="list-group">

                        <router-link v-for="airfield in airfields"
                                     class="list-group-item" :key="airfield.id"
                                     :to="{name:'adminAirfieldShow', params:{id:airfield.id}}"
                                     @click.native="show(airfield.id)"
                                     tag="li"
                                     style="cursor:pointer"
                        >
                            {{airfield.name|asideTitlify}}
                        </router-link>
            ...
            <div slot="asideMainContent">
                <div v-if="airfield.id">
                    <div class="card-header d-flex justify-content-between flex-wrap">
                        <span>{{airfield.name}}</span>
                        <div v-if="airfield.id">
                            <router-link :to="{name:'adminAirfieldEdit', params:{id:airfield.id}}">
                                <i class="mdi mdi-edit"></i>
                            </router-link>
                            <a class="pl-2 pr-2 default-color" @click.prevent="deleteMe(airfield.id)" href="">
                            ...
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <p class="cat_desc" v-if="airfield.created_at">
                                            <span>Date Created </span> <br>
                                            <strong>{{$root.parseDate(airfield.created_at)}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="airfield.updated_at">
                                            <span>Date Modified </span> <br>
                                            <strong>{{$root.parseDate(airfield.updated_at)}}</strong>
                                        </p>
                                        <div class="cat_desc">
                                            <span>Status </span> <br>
                                            <div class="switch-button switch-button-success switch-button-xs activeSwitch" >
                                                <input
                                                        type="checkbox"
                                                        :checked="airfield.is_active"
                                                        name="swt1"
                                                        :id="airfield.id"
                                                        @click.prevent="$root.changeIsBooleanStatus(airfield, 'is_active', 'App-Airfield')"
                                                >
                                                <span><label :for="airfield.id"></label></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
        ...
        methods:{
            getList(val){
                ...
                axios.get('/airfield-list',{
                    ...
                    this.airfields = response.data.data
            ...
            show(id){
                axios.get('/admin/airport/airfield/'+id,{

                }).then(response=>{
                    this.airfield = response.data
                })
            },
            deleteMe(id){
                this.$root.confirmationDelete().then(val => {
                    if (val) {
                        axios.delete('/admin/aiport/airfield/' + id).then(response => {
