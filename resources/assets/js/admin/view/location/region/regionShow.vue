    ...
    <div>
        ...
        <admin-aside>
            ...
            <div slot="asideContent">
                <div class=" aside_bar">
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <router-link class="btn btn-primary mr-1"
                                     :to="{name:'adminRegionCreate'}" v-if=" $root.$data.createPermission">
                            +Add
                        </router-link>
                ...
                <div class="listContent" ref="listHeight" :style="" v-if="regions.length">
                    <ul class="list-group">

                        <router-link v-for="region in regions"
                                     class="list-group-item" :key="region.id"
                                     :to="{name:'adminRegionShow', params:{id:region.id}}"
                                     @click.native="show(region.id)"
                                     tag="li"
                                     style="cursor:pointer"
                        >
                            {{region.name|asideTitlify}}
                        </router-link>
                    </ul>
                </div>
            ...
            <div slot="asideMainContent">
                <div v-if="region.id">
                    <div class="card-header d-flex justify-content-between flex-wrap">
                        ...
                        <div v-if="region.id">
                            <router-link :to="{name:'adminRegionEdit', params:{id:region.id}}">
                                <i class="mdi mdi-edit"></i>
                            </router-link>
                            <a class="pl-2 pr-2 default-color" @click.prevent="deleteMe(region.id)" href="">
                    ...
                    <div class="card-body  card-header-contrast">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-body">
                                        <p class="cat_desc" v-if="region.name">
                                            <span>Name </span> <br>
                                            <strong>{{region.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="region.continent.name">
                                            <span>Continent </span> <br>
                                            <strong>{{region.continent.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="region.created_at">
                                            <span>Date Created </span> <br>
                                            <strong>{{$root.parseDate(region.created_at)}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="region.updated_at">
                                            <span>Date Modified </span> <br>
                                            <strong>{{$root.parseDate(region.updated_at)}}</strong>
                                        </p>
                                        <div class="cat_desc">
                                            <span>Status </span> <br>
                                            <div class="switch-button switch-button-success switch-button-xs activeSwitch" >
                                                <input
                                                        ...
                                                        name="swt1"
                                                        :id="region.id"
                                                        @click.prevent="$root.changeIsBooleanStatus(region, 'is_active', 'App-Region')"
                                                >
                                                <span><label :for="region.id"></label></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
    export default {
        ...
        methods:{
            ...
            show(id){
                axios.get('/admin/location/region/'+id,{
            ...
            deleteMe(id){
                this.$root.confirmationDelete().then(val => {
                    if (val) {
                        axios.delete('/admin/location/region/' + id).then(response => {
