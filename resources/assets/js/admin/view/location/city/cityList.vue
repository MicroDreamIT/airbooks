<template>
    <div class="card card-table">
        <data-table-join
                ...
                :search="true"
                :columns="columns"
                :addLink="'adminCityCreate'"
                :tableHeadline="'Cities'"
                :searchField="'cities.name,states.name'"
                :modelName="'city'"
                :ajax="{
                            url: url
                        }"
        >
            <template slot="items" slot-scope="props">
                ...
                <td>
                    <router-link :to="{name:'adminCityShow', params:{id:props.data.id}}">
                        {{props.data.name}}
                    </router-link>
                </td>
                <td>
                    <router-link :to="{name:'adminCityShow', params:{id:props.data.id}}">
                        {{props.data.states_name}}
                    </router-link>
                </td>
                <td>
                    <div class="switch-button switch-button-success switch-button-xs activeSwitch"  v-if="$root.$data.editPermission">
                        <input
                                ...
                                name="swt1"
                                :id="props.data.id"
                                @click.prevent="$root.changeIsBooleanStatus(props.data, 'is_active', 'App-City')"
                        >
                ...
                <td>
                  <div class="adjustAction">
                      <router-link :to="{name:'adminCityEdit', params:{id:props.data.id}}" v-if="$root.$data.editPermission">
