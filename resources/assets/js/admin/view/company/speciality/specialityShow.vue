    ...
    <div>
        ...
        <admin-aside>
            ...
            <div slot="asideContent">
                <div class=" aside_bar">
                    ...
                    <div class="d-flex justify-content-end">
                        ...
                        <router-link class="btn btn-primary mr-1"
                                     :to="{name:'adminSpecialityCreate'}" v-if=" $root.$data.createPermission">
                            +Add
                        </router-link>
                ...
                <div class="listContent"  v-if="specialities.length">
                    <ul class="list-group">

                        <router-link v-for="speciality in specialities"
                                     class="list-group-item" :key="speciality.id"
                                     :to="{name:'adminSpecialityShow', params:{id:speciality.id}}"
                                     @click.native="show(speciality.id)"
                                     tag="li"
                                     style="cursor:pointer"
                        >
                            {{speciality.name|asideTitlify}}
                        </router-link>
            ...
            <div slot="asideMainContent">
                <div v-if="speciality.id">
                    <div class="card-header d-flex justify-content-between flex-wrap">
                        <span>{{speciality.name}}</span>
                        <div v-if="speciality.id">
                            <router-link :to="{name:'adminSpecialityEdit', params:{id:speciality.id}}">
                                <i class="mdi mdi-edit"></i>
                            </router-link>
                            <a href="" class="pl-2 default-color" @click.prevent="deleteMe(speciality.id)">
                    ...
                    <div class="card-body  card-header-contrast">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-body">
                                        <p class="cat_desc" v-if="speciality.name">
                                            <span>Name  </span> <br>
                                            <strong>{{speciality.name}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="speciality.created_at">
                                            <span>Date Created </span> <br>
                                            <strong>{{$root.parseDate(speciality.created_at)}}</strong>
                                        </p>
                                        <p class="cat_desc" v-if="speciality.updated_at">
                                            <span>Date Modified </span> <br>
                                            <strong>{{$root.parseDate(speciality.updated_at)}}</strong>
                                        </p>
                                        <div class="cat_desc">
                                            <span>Status </span> <br>
                                            <div class="switch-button switch-button-success switch-button-xs activeSwitch" >
                                                <input
                                                        type="checkbox"
                                                        :checked="speciality.is_active"
                                                        name="swt1"
                                                        :id="speciality.id"
                                                        @click.prevent="$root.changeIsBooleanStatus(speciality, 'is_active', 'App-Speciality')"
                                                >
                                                <span><label :for="speciality.id"></label></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script>
    export default {
        ...
        methods:{
            ...
            show(id){
                axios.get('/admin/company/speciality/'+id,{
            ...
            deleteMe(id){
                this.$root.confirmationDelete().then(val => {
                    if (val) {
                        axios.delete('/admin/company/speciality/' + id).then(response => {
