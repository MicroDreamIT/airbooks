<template>
    <div class="card card-table">
        <data-table
                ...
                :search="true"
                :columns="columns"
                :addLink="'adminCmsCreate'"
                :tableHeadline="'CMS'"
                :searchField="'title,sub_title'"
                :modelName="'cms'"
                :ajax="{
                            url: url
                        }"
        >
            <template slot="items" slot-scope="props">
                ...
                <td>
                    <router-link :to="{name:'adminCmsShow', params:{id:props.data.id}}">
                        {{props.data.title}}
                    </router-link>
                </td>
                <td>
                ...
                <td>
                    <div class="switch-button switch-button-success switch-button-xs activeSwitch"  v-if="$root.$data.editPermission">
                        <input
                                ...
                                name="swt1"
                                :id="props.data.id"
                                @click.prevent="$root.changeIsBooleanStatus(props.data, 'is_active', 'App-Cms')"
                        >
                ...
                <td>
                    <router-link :to="{name:'adminCmsEdit', params:{id:props.data.id}}" v-if="$root.$data.editPermission">
