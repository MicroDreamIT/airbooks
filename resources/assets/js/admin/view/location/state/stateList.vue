<template>
    <div class="card card-table">
        <data-table-join
                ...
                :search="true"
                :columns="columns"
                :addLink="'adminStateCreate'"
                :tableHeadline="'States'"
                :searchField="'states.name,countries.name'"
                :modelName="'state'"
                :ajax="{
                            url: url
                        }"
        >
            <template slot="items" slot-scope="props">
                ...
                <td>
                    <router-link :to="{name:'adminStateShow', params:{id:props.data.id}}">
                        {{props.data.name}}
                    </router-link>
                </td>
                <td>
                    <router-link :to="{name:'adminStateShow', params:{id:props.data.id}}">
                        {{props.data.countries_name}}
                    </router-link>
                </td>
                <td>
                    <div class="switch-button switch-button-success switch-button-xs activeSwitch" v-if="$root.$data.editPermission">
                        <input
                                ...
                                name="swt1"
                                :id="props.data.id"
                                @click.prevent="$root.changeIsBooleanStatus(props.data, 'is_active', 'App-State')"
                        >
                ...
                <td>
                    <div class="adjustAction">
                        <router-link :to="{name:'adminStateEdit', params:{id:props.data.id}}" v-if="$root.$data.editPermission">
